...
> **Claude Code**: Инструменты вызываются с префиксом `mcp__figma__`, например:
> - `get_figma_data` → `mcp__figma__get_figma_data`
> - `download_figma_images` → `mcp__figma__download_figma_images`
>
> Префикс формируется как `mcp__[имя_сервера]__[инструмент]`, где имя сервера — ключ из конфига (`"Figma"`).
